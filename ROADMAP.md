...
- [ ] **F3-G1** `template-authoring-convention` `lint_template_file` malformed-frontmatter diagnostic — `tools/lint-frontmatter.py:lint_template_file` calls the `parse_frontmatter` wrapper that discards `parse_errors`. On an unclosed `---` delimiter, the user gets "missing required field 'object_type'" instead of "malformed frontmatter delimiter." Observable behavior (non-zero exit) is correct; only the diagnostic is misleading. **When to address:** when adopters report misleading error messages, OR when refactoring the linter to share a single entry point between default and `--check-template` modes. Surface `parse_errors` from `_fm_parse_file` before the required-key check. Sub-task of `template-authoring-convention`.

- [ ] **F3-G2** `test_templates_instantiate.py` sub-directory glob currently a no-op — `pathlib.Path("templates").glob("*/README.md")` discovers only `templates/_meta/README.md`, which sits in the SKIP set. The glob first becomes load-bearing when F3.7 (Initiative) or F3.9 (Handoff Packet) lands a folder template. **When to address:** when F3.7 or F3.9 lands. Confirm the first folder-template README is picked up by the glob and gated by `--check-template`. Sub-task of `template-authoring-convention`.
