...
2. **A canonical product/business ontology** — eight domains, 84 documented typed objects (76 atomic + 8 composite), traceability rules from Market → Customer → Problem → Capability → Requirement → Outcome → Launch → Measurement.
...
- **Canonical product/business ontology** — eight domains, 84 documented typed objects (76 atomic + 8 composite), universal metadata schema. Every product artifact declares its `object_type:` and links into the traceability chain.
