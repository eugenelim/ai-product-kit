...
1. **Explicit speaker tags.** Lines prefixed with `Speaker A:` / `Speaker B:`, `Customer:` / `Interviewer:`, or named-speaker tags (`Priya:`) — accept the tags as authoritative. Map the customer-side tag to the interviewee; map the interviewer-side tag to the named interviewer in the optional `interviewer:` input. **Multi-customer case:** when explicit tags identify more than one distinct *customer-side* speaker in the same transcript (e.g., `Customer A:` and `Customer B:`), do NOT proceed. This is the roundtable case described in §"When this skill is wrong"; surface the multi-customer ambiguity and ask the orchestrator which speaker to code. One transcript ⇒ one snapshot ⇒ one customer per the framework's evidentiary contract.
