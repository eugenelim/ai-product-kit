...
*Worked example.* A PM runs a single A/B test on a checkout-flow change and observes p=0.04 against a predeclared 0.05 threshold. The correct Learning Memo entry is `status: survived` — one test, one threshold, one survival. It is **not** `status: validated`; the kit's vocabulary does not contain that value, by design. If the same team then cites "we validated checkout" in three subsequent decisions (pricing, onboarding, a launch-readiness review), they have committed the soft-confirm failure mode: a single survival has been laundered into a confirmation it cannot bear. True validation would require the test to be re-run on different cohorts, at progressively tighter thresholds, across distinct traffic conditions (mobile vs. desktop, new vs. returning users, peak vs. off-peak load), and survive each. Survival is per-test; validation is a pattern of surviving tests no one of them establishes alone.
...
*Product analogy.* Consider a feature that "works for 99% of users." In confirmation arithmetic this sounds overwhelming — 99 successes for every 1 failure. But suppose the 1% is a vulnerable subset (users on assistive tech, users in a regulated jurisdiction, users on the lowest-tier plan) and the failure mode is catastrophic (data loss, account lockout, regulatory exposure). One confirmed catastrophic-failure path is a kill-the-feature signal; 99% success does not offset it. The asymmetry is not symmetric arithmetic. One demonstrated kill mechanism is structurally stronger evidence than many demonstrated success mechanisms, because product failure modes **compound** (a data-loss bug becomes a support escalation becomes a churn event becomes a public incident) and confirmation does not (a successful checkout does not strengthen the next successful checkout). The kit's bias toward kill-rate over survival-rate is the operational expression of this asymmetry.

## Common failure modes

- **The "moving threshold"** — the threshold gets relaxed after a borderline result so the assumption survives. Defeats the entire mechanism; the hook can't catch this because the rationalization happens in commit messages and Slack threads, not in the threshold file. *Clean threshold revision:* declared **before** the result lands, with an explicit "I am changing the bar because the test conditions differed from spec — here is the deviation (e.g., the cohort skewed 80% mobile when the spec assumed a 50/50 split)" rationale recorded in the threshold file. *Dirty threshold revision:* edited **after** the result lands, with no rationale, with edits scoped precisely to make the assumption "survive" (0.05 → 0.06 the day p=0.057 lands). The mitigation has two layers: convention says any threshold edit carries a written rationale in the threshold file; the human gate says any **post-result** threshold edit requires explicit reviewer sign-off recorded in the experiment's `human_owned_decisions:` field, naming the reviewer and the date. Without that signed entry, the survival does not count and downstream artifacts that cite it are out of compliance with the audit chain.
