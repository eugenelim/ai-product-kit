...
| `/opportunity-narrative` | SC | YOU | Opportunity Narrative (Opportunity \| Adapted) | Customer-shaped one-pager crossing Handover 2 alongside the OST; refuses to draft unless `chosen_opportunity:` is set AND resolves to a real OST node; cites Direct Quotes verbatim; never fabricates; emits NEXT /assumption-test | shipped (P2.12, 2026-05-28) |
| `/discovery-update` | SC | YOU (weekly) | Discovery Update (Insight \| Adapted) | Weekly stakeholder digest; six H2 sections (Headline / By the numbers / What changed in the OST / What we learned / What's blocking / Next week); surfaces continuous-discovery cadence drift on quiet weeks; five role-mode mechanical anchors; terminal (no NEXT) | shipped (P2.14, 2026-05-28) |
| `/audit-discovery-coherence` | SC | YOU | Audit Report | Five Discovery-coherence rules (OST→intent, intent→OST, chosen_opportunity resolution, evidence_basis sourcing, outcome alignment); verdicts clean/drift/broken/insufficient-data; precedence broken>drift>insufficient-data>clean; per-verdict NEXT chain; --write persists to docs/audits/ | shipped (P2.11, 2026-05-28) |
...
| `roadmap-skeptic` | AG | CLAUDE (dispatch) | Review | Phase-4 specialist reviewer (sibling of `adversarial-reviewer` / `quality-engineer`) — classifies dated promises as commitments (owner + dependency map + falsifiable delivery condition) vs bets (any one missing); six-category output covers bets, settled-assumptions, scope-vs-parent, placeholder `human_owned_decisions`, dated language without evidence basis, and ungrounded capabilities/features. Two hard `block` predicates (must-test-before-shipping treated as settled; Draft Vision missing approvals) plus a discretionary block. | shipped 2026-05-23 (P4.16) |
