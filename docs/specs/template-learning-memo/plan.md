...
- 2026-05-22 (EXECUTE deviation): Flattened `test.predeclared_threshold` from `{success, falsification}` nested map to a single placeholder scalar. **Why:** the kit's frontmatter parser (`scripts/lib/frontmatter.py`) is documented as supporting only "single-level nested maps; list-of-maps at depth 2"; HANDOVERS-3's `test.predeclared_threshold.{success, falsification}` requires depth-3 nesting which the parser silently flattens to an empty string, failing `--check-template`. Trade-off: the learning memo loses the structured split between success and falsification criteria, but the **hook-enforced** version of those fields lives in `templates/experiment/experiment.md` at top level (depth 1), which parses correctly and is what the `assumption-threshold-lock` hook actually polices. T3's nested-key assertion (`fm['test']['predeclared_threshold']` is a dict) is therefore **superseded** by this deviation — the YAML key exists but holds a scalar placeholder pointing at the linked `experiment.md`. The underlying parser limitation is a separate, follow-up concern (out of scope for F3.5); enhancing it would belong in a future kit-tooling spec.
