...
- 2026-05-23: EXECUTE-phase drift fix: T14's awk in spec.md had a broken `END{if(NR>0) exit 1}` clause that fires after the in-body `exit 0` and inverts the test. Replaced with a `found` flag pattern that catches the "neither section present" failure mode without overriding the success path. No structural change to the convention; the on-disk H3 ordering is unaffected.
- 2026-05-23: REVIEW-phase fix (NF2 from adversarial-reviewer): the template-path regex in `test_inscope_commands_cite_template_path` was `templates/[a-z0-9-]+(\.md|/)`, which would have failed for augmenting commands that cite child files inside folder templates (e.g., `templates/initiative/context-map.md`). Widened to `templates/(?:[a-z0-9-]+/)*[a-z0-9-]+(\.md|/)` in both the spec (§"Outputs" item 4) and the on-disk test. No on-disk effect on the current pass count (still 3 pass / 35 skip).
- 2026-05-23: CAPTURE-phase drift fix: `.gitignore` excluded `docs/specs/**/notes/` broadly as "session scratch", but multiple shipped specs (`agent-traceability-walker`, `audit-completeness-script`, `claude-settings-hooks-wiring`, `hook-mode-guard`, `lint-frontmatter-ai-assistance-allowed`, `ontology-classifier-skill`, `reconcile-existing-components`) reference tracked `notes/*.md` deliverables that the broad exclusion silently blocked from landing. This spec's contract (T12, T12b) mandates `notes/per-command-spec-checklist.md` ships in the repo. Narrowed the gitignore to exclude only `state.json` (the actual session-scratch artifact, which has its own dedicated line) — `notes/` files are now tracked by default, matching the `.claude/skills/work-loop/SKILL.md` Step 2.3 instruction that scope-creep findings get recorded under `notes/`.
