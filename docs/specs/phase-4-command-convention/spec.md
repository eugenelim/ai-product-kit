...
4. `scripts/tests/test_phase4_command_shape.py` — new pytest file. Test cases:
   ...
   - `test_inscope_commands_cite_template_path` — for each existing in-scope command, search the body for at least one path matching `templates/[a-z0-9-]+(\.md|/)` AND assert that path exists in the repo. Catches drift where a command points to a template that doesn't exist (e.g., a renamed F3 template).
...
- `T14` — Convention sub-section appears AFTER the §"Templates — `templates/<slug>.md`" sub-section in `docs/CONVENTIONS.md`. `awk '/^### Templates — \`templates\/<slug>\.md\`/{seen=1} /^### Phase-4 Template-Fill Commands/{if(seen) exit 0; else exit 1} END{if(NR>0) exit 1}' docs/CONVENTIONS.md` exits 0. Mirrors F3's T14d positional check.
